...
**Estado y seguridad actuales**: API REST completamente funcional con **JWT + BCrypt + Spring Security** (Enero 2026). Login genera token JWT válido por 1 hora, todos los endpoints protegidos requieren token, contraseñas hasheadas con BCrypt. Arquitectura stateless (sin sesiones de servidor). Errores centralizados con `@ControllerAdvice` y `BusinessException` para respuestas JSON uniformes.
