...
- [Plataforma web de pedidos para panadería](#plataforma-web-de-pedidos-para-panadería)
        - [Enlace a documentación: doc](#enlace-a-documentación-doc)
        - [Enlace repo GitLab: RepoGitLab](#enlace-repo-gitlab-repogitlab)
...
**Estado y seguridad actuales**: API REST completamente funcional con **JWT + Refresh Tokens + BCrypt + Spring Security** (Febrero 2026). Sistema de autenticación con access token (corta duración) y refresh token (7 días) persistido en BD con rotación automática. Rate limiting en `/auth/login` (5 intentos/15min por email) para prevenir fuerza bruta. Actuator health endpoint disponible. Contraseñas hasheadas con BCrypt. Arquitectura stateless (sin sesiones de servidor). Errores centralizados con `@ControllerAdvice` y `BusinessException` para respuestas JSON uniformes.

**Diagrama de arquitectura general (actual)**: Frontend estático (HTML/JS/Bootstrap) servido por Nginx, API REST en Spring Boot, MySQL persistente. El token JWT se guarda en `localStorage` y se envía en `Authorization: Bearer <token>` en cada petición. Tráfico expuesto por Nginx en `8081`.
...
- Flujos mínimos: registrar usuario (`/auth/register`) → crear pedido → revisar pedido; admin → crear producto → actualizar stock → cambiar estado pedido.
- Manejo de errores: provocar una validación fallida y comprobar que la respuesta incluye `timeStamp`, `status`, `error`, `message` y `path`.
